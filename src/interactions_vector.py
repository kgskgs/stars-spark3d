...
def calc_gforce_cartesian(in_name, in_path, limit, G, part="id"):
    ...
    df_clust = utils.load_df(
        in_name, in_path, limit=limit, schema=schemas.clust_input, part=part)

    assemblerR = VectorAssembler(inputCols=['x', 'y', 'z'], outputCol="r")
    assemblerV = VectorAssembler(inputCols=['vx', 'vy', 'vz'], outputCol="v")

    df_clust = assemblerR.transform(assemblerV.transform(
        df_clust)).drop('x', 'y', 'z', 'vx', 'vy', 'vz')

    df_clust_cartesian = df_clust.crossJoin(
        df_clust.selectExpr(*["`{0}` as {0}_other".format(x)
                              for x in df_clust.columns])
    ).filter("id != id_other")
...
def sum_gforce(in_name, in_path=None, part="id"):
    ...
    if in_path:
        df_gforce_cartesian = utils.load_df(
            in_name, in_path, schema=schemas.v_dist_gforce_cartesian, part=part)
    else:
        df_gforce_cartesian = in_name

    rdd_gforce = df_gforce_cartesian.rdd.map(
        lambda x: (x['id'], (x['gforce'], x['gv'])))

    rdd_gforce = rdd_gforce.reduceByKey(
